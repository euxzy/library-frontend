...
@section('content')
  <section class="my-5 py-5">
    <form action="{{ route('books.update', $book['id']) }}" method="POST" enctype="multipart/form-data">
      @csrf
      <label for="name">Judul Buku : <input type="text" name="name" id="name" value="{{ $book['name'] }}"></label>

      <label for="id_author">Penulis : <input type="number" name="id_author" id="id_author" value="{{ $book['author']['name'] }}"></label>
